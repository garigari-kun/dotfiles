...
let BackSpaceInVisualMode = class BackSpaceInVisualMode extends baseMotion_1.BaseMovement {
    constructor() {
        super(...arguments);
        this.modes = [mode_1.ModeName.Visual, mode_1.ModeName.VisualBlock];
        this.keys = ['<BS>'];
    }
    execAction(position, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            return configuration_1.configuration.whichwrap.includes('b')
                ? position.getLeftThroughLineBreaks()
                : position.getLeft();
        });
    }
};
BackSpaceInVisualMode = __decorate([
    base_1.RegisterAction
], BackSpaceInVisualMode);
...
exports.MoveUpByDisplayLine = MoveUpByDisplayLine;
...
let MoveNonBlankFirst = class MoveNonBlankFirst extends baseMotion_1.BaseMovement {
    ...
    execActionWithCount(position, vimState, count) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return new position_1.Position(count - 1, 0).obeyStartOfLine();
        });
    }
};
...
let MoveNonBlankLast = class MoveNonBlankLast extends baseMotion_1.BaseMovement {
    ...
    execActionWithCount(position, vimState, count) {
        return __awaiter(this, void 0, void 0, function* () {
            let stop;
            if (count === 0) {
                stop = new position_1.Position(textEditor_1.TextEditor.getLineCount() - 1, position.character).obeyStartOfLine();
            }
            else {
                stop = new position_1.Position(Math.min(count, textEditor_1.TextEditor.getLineCount()) - 1, position.character).obeyStartOfLine();
            }
            return {
